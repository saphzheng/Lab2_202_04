...
public class LineGraphView extends View
{
	...
	private final int AXIS_WIDTH = 300;
	...
	private void prepForData(Canvas canvas)
	{
		...
		canvas.drawText(Float.toString(maxY) + " m/s^2", 0, 30, graphPaint);
		canvas.drawText("-" + Float.toString(maxY) + " m/s^2", 0, HEIGHT, graphPaint);
		
		for(int i = 0; i < labels.size(); i++){
			canvas.drawText(labels.get(i) + ":", 0, 60 + i*30, graphPaint);
			canvas.drawLine(0, 65 + i*30, AXIS_WIDTH - 20, 65 + i *30, linePaints.get(i));
		}
	}
